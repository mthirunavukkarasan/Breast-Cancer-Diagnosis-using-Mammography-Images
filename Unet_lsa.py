...
def Unet_lsa(train_data, train_target, test_data,test_target):
    # Set some parameters
    IMG_SIZE = 256
    seed = 42
    rn.seed = seed
    ...
    ret_img = pred_img[:, :, :, 0]
    lss = []
    for a in range(len(ret_img)):
        lsa = chan_vese(ret_img[a], mu=0.25, lambda1=1, lambda2=1, tol=1e-3, max_iter=200, dt=0.5, init_level_set='checkerboard',
                   extended_output=True)
        lss.append(lsa)
    Eval = Segmentation_Evaluation(lss, test_target)
    return Eval, lss
